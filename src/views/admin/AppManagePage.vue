...
// 搜索条件
const searchParams = reactive<AppQueryDto>({
  pageNum: 1,
  pageSize: 10,
})
...
const fetchData = async () => {
  try {
    const res = await AppService.listAppVOByPageByAdmin({
      ...searchParams,
    })
...
const toggleFeatured = async (app: AppVO) => {
  ...
  try {
    const res = await AppService.updateAppByAdmin({
      id: app.id,
...
const deleteApp = async (id: number | undefined) => {
  ...
  try {
    const res = await AppService.deleteAppByAdmin({ id })
    if (res) {
